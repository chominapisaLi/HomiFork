...
if SERVER then
	...
	function JMod.EZ_ScroungeArea(ply, cmd, args)
		...
		print(name)
		if name ~= 'JMOD SURVIVAL' then return ply:PrintMessage(HUD_PRINTCENTER, "Поиск мусора здесь не доступен :(") end
		local Time = CurTime()
		...
		if not (Debug) then
			for k, pos in pairs(ScroungedPositions) do
				...
				if (DistanceTo < Range) then ply:PrintMessage(HUD_PRINTCENTER, "Поиск мусора здесь не доступен:(") return end
			end

			ply.NextScroungeTime = ply.NextScroungeTime or 0
			if ply.NextScroungeTime > Time then ply:PrintMessage(HUD_PRINTCENTER, "Подождите чуть чуть..") return end
