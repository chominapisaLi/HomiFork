...
function CLASS:EndRound(winner)
	if roundActiveName == "homicide_extended" and winner == 2 then
...
if SERVER then
	util.AddNetworkString("homicide_extended contr")

	net.Receive("homicide_extended contr",function(len,ply)
		...
		--[[for i,ply in pairs(getList(ply)) do
			net.Start("homicide_extended contr")
