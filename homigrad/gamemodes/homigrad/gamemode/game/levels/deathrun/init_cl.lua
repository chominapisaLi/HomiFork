...
function deathrun.HUDPaint_RoundLeft(white2,time)
    ...
    if startRound > 0 and lply:Alive() then
        --if playsound then
        --    playsound = false
        --    surface.PlaySound("snd_jack_hmcd_m_6.mp3")
        --end
        lply:ScreenFade(SCREENFADE.IN,Color(0,0,0,100),0.5,0.5)
        ...
        draw.DrawText( "DEATHRUN", "HomigradFontBig", ScrW() / 2, ScrH() / 8, Color( 155,55,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
