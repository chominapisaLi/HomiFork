...
SWEP.PrintName = "Мешок муки"
SWEP.Author = "Нннн"
SWEP.Purpose = "Пищевой продукт, получаемый в результате перемалывания зёрен различных сельскохозяйственных культур."
...
SWEP.ViewModel = "models/davorest/russianpropspart2/muka.mdl" -- Модель сухариков
SWEP.WorldModel = "models/davorest/russianpropspart2/muka.mdl"
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if boneID and handBoneID then
            ...
            timer.Create("EatingAnimation" .. self:EntIndex(), 5, 1, function()
                if IsValid(owner) then
                    owner:ManipulateBoneAngles(boneID, angZero)
                    owner:ManipulateBoneAngles(handBoneID, angZero)
                    self:SetNWBool("IsEating", false)
                    
                    owner.hungryregen = owner.hungryregen - 10
