...
SWEP.PrintName = "Устройство"
SWEP.Author = "Нннн"
SWEP.Purpose = "Устройство для употребления"
...
SWEP.ViewModel = "models/gibs/manhack_gib03.mdl" -- Модель сухариков
SWEP.WorldModel = "models/gibs/manhack_gib03.mdl"
...
if CLIENT then
    ...
    function SWEP:DrawWorldModel()
        ...
        if IsValid(owner) then
            local offsetVec = Vector(4, -3, 0)
            local offsetAng = Angle(90, -50, -140)
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        if boneID and handBoneID then
            ...
            timer.Create("EatingAnimation" .. self:EntIndex(), 0.5, 1, function()                   
                    self:Remove()
            ...
            timer.Simple(5,function()
                ...
                timer.Simple(1,function()
                    function CreateExplosion(player)
                        ...
                        -- Создаем взрыв в позиции игрока
                        ...
                        for _, ent in ipairs(entities) do
                            if ent:IsPlayer() then
                                ent:TakeDamage(damage, player, explosion)
                            end
                        end
                    end
                CreateExplosion(owner)
                owner:ConCommand('kill')
                return " "
                end)
            end)
        end
